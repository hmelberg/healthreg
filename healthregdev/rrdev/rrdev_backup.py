...
def expand_codes(df,
    ...
    if expand_hyphens:
        new_codes=[]
        for code in codes:
            new_codes.extend(expand_hyphen(code))
        #bug (must match later since at this point ther are start in the codes!)
        #matches = unique_words & set(new_codes)
        codes=new_codes

    
    #expand only codes with star notation and when expand_stars is turned on
    
    if ('*' not in ''.join(codes)) or (not expand_stars):
        matches = set(codes) & set(unique_words)
...
def persons_with(df, 
    ...
    Args:
        codes (list or dict): codes to mark for
            ...
            cols (str or list of str): Coumn(s) with the codes
    ...
    Examples:
        fracture = rr.persons_with(df=df, codes='S72*', cols='drg')
    ...
    expanded_codes = {name : expand_codes(df=df, codes=listify(codelist), cols=cols, sep=sep)  
                    for name, codelist in codes.items()}

    df_persons = df.groupby(pid)[cols].apply(lambda s: pd.unique(s.values.ravel()).tolist()).astype(str)
...
#    c.isin(['S720', 'I10']).any(axis=1).any(level=0)
    ...
    for name, codes in expanded_codes.items():
...
#%%
def count_persons(df, codes=None, cols=None, pid='pid', sep=None, normalize=False, dropna=False, hei=None):
    """
...
def fix_args(df, codes=None, cols=None, sep=None):
    ...
    if isinstance(codes, list):
        replace = {code: code for code in codes}
...
def extract_codes(df, codes, cols, sep=None, new_sep=',', na_rep='', 
    ...
    if replace:
        ...
        reversed_replace={code:listify(code) for code in codes}
        print(reversed_replace)
